...
## n2: Stable dev-build signing so FDA survives rebuilds
- **Did:** scripts/build.sh now signs Debug builds with a stable local identity instead of building unsigned. It looks up the login keychain (`security find-identity -v -p codesigning`), preferring `Apple Development` then `Developer ID Application`, and passes `CODE_SIGNING_ALLOWED=YES CODE_SIGN_STYLE=Manual CODE_SIGN_IDENTITY=<name> PROVISIONING_PROFILE_SPECIFIER=` to xcodebuild. With a real certificate the designated requirement is `identifier "dev.viraat.i2message" and certificate leaf[subject.CN] = "Apple Development: Viraat Das (VF44U5EPN6)"` — stable across rebuilds, so the TCC Full Disk Access grant persists. When no identity exists (CI, other machines) the script falls back to the prior `CODE_SIGNING_ALLOWED=NO` behavior. `I2MESSAGE_CODE_SIGN_IDENTITY` overrides the lookup (empty value forces the unsigned fallback). test.sh unchanged. Also repaired this workspace's inherited merge damage: resolved 2-sided jj conflicts in Components.swift/ConversationDetailView.swift by keeping the wrqklkvz tapback-pill side (the vnnsmyml "Mark MediaThumbnail.load result as sending" CI-fix commit was authored on a pre-pill tree and its merge tried to revert TapbackPill/RightClickCatcher; its actual `sending` fix was already present on both sides), and restored AppViewModel.currentUserReaction(on:) plus its test, which the same stale-tree merge silently deleted even though both merge parents contain them.
- **Interfaces:** scripts/build.sh (identity lookup + I2MESSAGE_CODE_SIGN_IDENTITY env override); README.md build/signing note. No project.yml or App/i2Message.xcconfig changes — the override is command-line-only so Xcode GUI behavior is untouched.
- **Verified:** Two consecutive ./scripts/build.sh runs with a code change between both signed as `Authority=Apple Development: Viraat Das (VF44U5EPN6)`, TeamIdentifier=3C4383262W, flags=0x10000(runtime) (not adhoc/linker-signed); `codesign --verify --deep --strict` passes; `I2MESSAGE_CODE_SIGN_IDENTITY= ./scripts/build.sh` builds successfully with the old adhoc fallback; ./scripts/test.sh TEST SUCCEEDED.
- **By:** n2 · 2026-07-12
