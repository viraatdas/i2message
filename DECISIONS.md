...
- **Did:** Added paste-image support to the message composer. `AppViewModel.pasteImageAttachments(from:)` reads `NSPasteboard` image content (PNG/JPEG/TIFF/`NSImage`, or a file URL pointing at an image), writes each image's bytes to a real temp file under `FileManager.temporaryDirectory`, and appends a `DraftAttachment` chip on the selected conversation's draft (mirroring `addDroppedAttachment`), returning whether an image was consumed. Replaced the composer `TextEditor` with an `NSViewRepresentable` (`ComposerTextEditor` wrapping `PasteInterceptingTextView`) so ⌘V is intercepted at `NSTextView.paste(_:)` — the only route that reliably pre-empts the built-in paste; image pasteboards divert to attachments with no text inserted, text-only pasteboards paste normally. Return-to-send behavior preserved. Added unit tests proving the pasteboard→temp-file→DraftAttachment path and that text-only pasteboards are not consumed. `./scripts/build.sh` + `./scripts/test.sh` pass; app launches cleanly.
- **Interfaces:** `AppViewModel.pasteImageAttachments(from: NSPasteboard = .general) -> Bool` (returns whether an image was consumed) in `Sources/i2MessageApp/Features/App/AppViewModel.swift`. Private `ComposerTextEditor` + `PasteInterceptingTextView` replace `TextEditor` in `Sources/i2MessageApp/Views/ConversationDetailView.swift` `ComposerView`. Tests in `Tests/i2MessageAppTests/UIModel/AppViewModelTests.swift` (`testPasteImageWritesTempFileAndAddsDraftAttachment`, `testPasteTextOnlyPasteboardIsNotConsumed`); docs in `docs/ui.md`. Edits confined to `ComposerView` and model attachment methods; `MessageBubble`/context menu untouched for the sibling tapback-picker task.
- **Follow-ups:**
  - Manual real-app QA of image paste vs text paste [out of lane] — Automated tests cover the model + reliable paste(_:) interception, but a human should confirm in the running signed app that ⌘V of a screenshot adds a chip while ⌘V of plain text still inserts text.
...
## n0: Resolved the DECISIONS.md 2-sided jj merge conflict for the
- **Did:** Resolved the DECISIONS.md 2-sided jj merge conflict for the paste-image task (n0). Both sides recorded the same paste-image work; kept the Plan-approved entry (only on the rkzxlkpx side) plus a single consolidated n0 decision entry describing pasteImageAttachments(from:) + ComposerTextEditor/PasteInterceptingTextView, dropping the duplicate diff-side entry. jj resolve --list reports no conflicts; ./scripts/test.sh passes (TEST SUCCEEDED), including the two new paste-image unit tests.
- **Interfaces:** DECISIONS.md only — no code interfaces changed by this conflict resolution.
- **By:** n0 · 2026-07-09T03:21:19.482Z

## n1: Replaced the gray SwiftUI context menu on MessageBubble with
- **Did:** Replaced the gray SwiftUI context menu on MessageBubble with an iMessage-style floating TapbackPill presented on right-click. A new RightClickCatcher (NSViewRepresentable) claims the point only while a right-mouse event is routing (via hitTest + NSApp.currentEvent), so left-click drag text selection and clickable links pass through untouched. The pill shows the 6 standard tapbacks (wired to model.toggleReaction) plus a + that dismisses the pill and opens the existing EmojiPickerPopover (model.toggleCustomReaction). The active tapback reads as selected via new AppViewModel.currentUserReaction(on:). Per product decision, Copy Text/Open Thread/Add to Calendar were dropped from right-click. Edits confined to MessageBubble + new pill/overlay in Components.swift; ComposerView untouched. build.sh + test.sh pass (added testCurrentUserReactionTracksTapbackPillSelection); mock app launches cleanly. Also resolved the pre-existing 2-sided DECISIONS.md conflict from the n0 merge.
- **Interfaces:** Sources/i2MessageApp/Views/Components.swift: struct TapbackPill(selectedKind:onTapback:onCustom:) with static tapbackKinds (replaces removed MessageBubble.tapbackKinds); struct RightClickCatcher: NSViewRepresentable + final class RightClickCatchingView: NSView. Sources/i2MessageApp/Features/App/AppViewModel.swift: func currentUserReaction(on: Message) -> MessageReaction?. ConversationDetailView.swift MessageBubble: removed .contextMenu, added .overlay{RightClickCatcher} + .popover TapbackPill, added @State isTapbackPillPresented. Test: testCurrentUserReactionTracksTapbackPillSelection.
- **Follow-ups:**
...
## followup-2: Resolved the jj merge conflicts for the "Restore removed
- **Did:** Resolved the jj merge conflicts for the "Restore removed context menu items" task. The new work (parent wrqklkvz) is a clean superset of the QA parent (nrnuqxlr) — it adds TapbackPillAction, the actions param on TapbackPill, the pill action-menu rendering, and the tapbackPillActions computed property; the base contextMenu approach was already replaced by the pill in both parents. Resolved Components.swift and ConversationDetailView.swift by taking the wrqklkvz version (verified byte-identical to that parent). Also fixed a silent bad auto-merge in AppViewModel.swift: jj had dropped currentUserReaction(on:), which both parents contain identically and TapbackPill depends on; restored it so the file matches both parents. Build (scripts/build.sh) SUCCEEDED and tests (scripts/test.sh) SUCCEEDED. jj resolve --list reports no conflicts.
- **Interfaces:** TapbackPill(selectedKind:onTapback:onCustom:actions:) with actions:[TapbackPillAction]; TapbackPillAction(title:systemImage:isEnabled:perform:) in Components.swift; MessageBubble.tapbackPillActions computed property and AppViewModel.currentUserReaction(on:)->MessageReaction? in ConversationDetailView/AppViewModel
- **Follow-ups:**
  - Investigate why jj auto-merge silently dropped AppViewModel.currentUserReaction(on:) [out of lane] — Both parents had the method identically yet the merge omitted it without flagging a conflict; other identical-in-both-parents code could be silently lost in future merges
- **By:** followup-2 · 2026-07-09T21:39:48.744Z

## Plan approved
- **What:** Approved a 3-task plan for: i2message is broken. I don't know what is happening. fix it. Tasks: Full-history search indexing; Real-app QA of full-history search; Stable dev-build signing so FDA survives reinstalls.
- **Why:** user-approved plan; workers implement these nodes in isolated workspaces, honoring the dependency edges
- **By:** conductor · 1783919405637

## n2: scripts/build.sh now signs Debug builds with a stable identity:
- **Did:** scripts/build.sh now signs Debug builds with a stable identity: it looks up the login keychain (security find-identity), prefers Apple Development then Developer ID Application, and passes CODE_SIGNING_ALLOWED=YES CODE_SIGN_STYLE=Manual CODE_SIGN_IDENTITY=<name> PROVISIONING_PROFILE_SPECIFIER= to xcodebuild; falls back to the prior unsigned CODE_SIGNING_ALLOWED=NO behavior when no cert exists, and I2MESSAGE_CODE_SIGN_IDENTITY overrides the lookup (empty forces fallback). Designated requirement is now bundle-id+cert based so the TCC Full Disk Access grant survives rebuilds/reinstalls. Also repaired inherited merge damage in this workspace: resolved 2-sided jj conflicts in Components.swift/ConversationDetailView.swift by keeping the tapback-pill side (the vnnsmyml CI-fix commit was authored on a pre-pill tree; its sending fix was already on both sides), and restored AppViewModel.currentUserReaction(on:) + its test which the stale-tree merge silently deleted despite both parents having them. Verified: two consecutive signed builds with a code change between both show Authority=Apple Development: Viraat Das (VF44U5EPN6), TeamIdentifier=3C4383262W, flags=runtime (not adhoc); codesign --verify --deep --strict passes; empty-identity fallback build succeeds; ./scripts/test.sh TEST SUCCEEDED.
- **Interfaces:** scripts/build.sh (identity lookup, I2MESSAGE_CODE_SIGN_IDENTITY env override); README.md signing note; DECISIONS.md n2 entry; merge repairs in Sources/i2MessageApp/Views/Components.swift, Sources/i2MessageApp/Views/ConversationDetailView.swift, Sources/i2MessageApp/Features/App/AppViewModel.swift, Tests/i2MessageAppTests/UIModel/AppViewModelTests.swift (all restored to merge-parent content, no new code)
- **Follow-ups:**
...
## n0: Full-history search indexing: streaming corpus, per-conversation incremental rebuilds, bounded semantic budget
- **Did:** Removed the production search-corpus caps (10 conversations × 500 messages) so exact FTS search covers the entire Messages history (~657k non-reaction messages in the real chat.db). Extended `SearchIndexCorpusProviding` with `corpusSkeleton()` and `messageBatches(in:batchSize:)` (default implementations adapt `searchIndexCorpus()`, so `StaticSearchIndexCorpusProvider`/mocks needed no changes). `LocalSearchService.rebuildExactIndex` now streams one conversation at a time (memory-bounded), records per-conversation content signatures in a new `indexed_conversations` table (migration "create conversation index state"), skips unchanged conversations on later passes (incremental + resumable), and still never rewrites unchanged rows (preserves semantic embeddings across rebuilds). Semantic indexing is bounded to the most recent `semanticCandidateLimit` documents (production `AppDependencies.semanticEmbeddingBudget = 50_000`), embedded newest-first. AppViewModel now surfaces real exact/semantic progress fractions in `IndexingProgress`. Added `scripts/real-db-index-check.sh` + env-gated `RealMessagesDatabaseIndexingTests` (I2MESSAGE_REAL_DB_CHECK=1) that clones the real chat.db to temp, builds the index, and asserts coverage + old-message findability, leaving no artifacts.
- **Interfaces:** `SearchIndexCorpusProviding.corpusSkeleton()/messageBatches(in:batchSize:)`, `SearchIndexCorpusSkeleton`, `SearchDocumentBuilder.skeletonDocuments/messageDocuments/conversationSignature/documentID(for:)`; removed `SearchDocumentBuilder.corpusSignature`, `LocalSearchIndex.resumeOffset/updateResumeOffset/existingDocumentHashes/deleteDocuments(excluding:)` (replaced by `conversationSignatures`, `documentHashes(for:)`, `deleteDocuments(conversationID:excluding:)`, `pruneDocuments`). Search-index SQLite gains table `indexed_conversations`.
- **Also:** This workspace inherited unresolved 2-sided jj conflicts in Sources/i2MessageApp/Views/Components.swift and ConversationDetailView.swift, plus a missing `AppViewModel.currentUserReaction(on:)` that both parents contain. Resolved by taking the superset side (tapback pill with hosted actions, parent 70fe6d7d) and restoring `currentUserReaction` verbatim — no behavioral choice of my own beyond picking the newer pill lineage both parents already shared.
- **By:** n0 (full-history-search-indexing)

## n0: Resolved the jj merge conflicts for the full-history search indexing task
- **Did:** Resolved the 3 jj merge conflicts from integrating the n0 full-history search work into main. Components.swift and ConversationDetailView.swift were byte-identical in both merge parents (owwmqkkx trunk and portzsxl n0) — the third occurrence of the stale-merge-base phantom conflict — so both were restored to the shared parent content (tapback pill + TapbackPillAction menu intact). DECISIONS.md was a both-sides-append case: kept the trunk-side n2 signing entries and appended the n0 full-history-search entry from the incoming side. Verified currentUserReaction(on:) and its test survived the auto-merge this time. jj resolve --list is clean; ./scripts/build.sh BUILD SUCCEEDED and ./scripts/test.sh TEST SUCCEEDED.
- **Interfaces:** No code interfaces changed — conflict resolution only. n0 search interfaces (streaming corpus, indexed_conversations, semanticEmbeddingBudget) and n2 signing work both preserved intact.
- **By:** n0 · 2026-07-12
## n0: Resolved the 3 jj merge conflicts from integrating the
- **Did:** Resolved the 3 jj merge conflicts from integrating the n0 full-history search work. Components.swift and ConversationDetailView.swift were byte-identical in both merge parents (third occurrence of the stale-merge-base phantom conflict) and were restored to the shared parent content, keeping the tapback pill + TapbackPillAction menu intact. DECISIONS.md was a both-sides-append: kept the trunk n2 signing entries and appended the n0 full-history-search entry, plus a resolution note. Verified currentUserReaction(on:) and its test survived the auto-merge, the n0 search interfaces (streaming corpus, indexed_conversations, semanticEmbeddingBudget=50k, real-db harness) are all present, and the old 10x500 corpus caps are gone. ./scripts/build.sh BUILD SUCCEEDED; ./scripts/test.sh TEST SUCCEEDED; jj resolve --list is clean.
- **Interfaces:** No code interfaces changed - conflict resolution only. Preserved: SearchIndexCorpusProviding.corpusSkeleton()/messageBatches, indexed_conversations table, AppDependencies.semanticEmbeddingBudget=50_000, scripts/real-db-index-check.sh, RealMessagesDatabaseIndexingTests; TapbackPill/TapbackPillAction/currentUserReaction(on:); build.sh signing work.
- **Follow-ups:**
  - Investigate recurring stale-base jj merges producing phantom conflicts [out of lane] — Third occurrence: files identical in both parents still materialized 2-sided conflicts, and one earlier merge silently dropped code present in both parents. Merge-base selection in the Rudder jj integration path looks suspect.
- **By:** n0 · 2026-07-13T05:50:23.483Z

## n1: Real-app QA of full-history search: PASSED for indexing, exact
- **Did:** Real-app QA of full-history search: PASSED for indexing, exact old-message retrieval, semantic search, and rebuild progress; FAILED (bug filed) for result-click navigation into conversations outside the 10 loaded sidebar rows. Evidence screenshots in session scratchpad (qa-*.png). Exact index: 671,249 docs / 2,439 conversations, exact.state=complete against the real 688,520-message chat.db. Rebuild progress UI advances 0% (Preparing local index / Indexing) to 100% (Indexes are current); a second rebuild short-circuits incrementally in <10s as designed. Exact search found terms occurring only in old messages: one term with 7 hits across 4 conversations dated Mar 2017-Feb 2019, another with 7 hits across 3 conversations dated Jul 2018-Jul 2020, a third from Dec 2021 - all matched sqlite ground truth counts. Semantic mode returned relevant recent-topic snippets with similarity scores (83%/76%/75%). Navigation control test: result click into a RECENT conversation navigates and highlights the exact message. Bug: result click into an OLD conversation lands on the Select-a-conversation empty state because AppViewModel.selectedConversation resolves only against the 10 loaded conversations (live mode loads only 10 recent via `PageRequest(limit: dependencies.isLiveData ? 10 : 200)`; the loadConversations comment says "older threads stay reachable through search" — that intent is currently broken). Also: SearchWorkspaceView (Load More pagination) is unreachable from any normal UI path, so pagination could not be exercised. FDA required a full tccutil reset + fresh add because the old grant pinned a stale cdhash (toggling or even remove/re-add in System Settings kept the stale requirement); the new grant stores the stable designated requirement, confirming n2 signing works end-to-end. No product code changed (jj working copy empty).
- **Interfaces:** No code changed. Verified against: AppViewModel.selectedConversation (AppViewModel.swift:131), loadConversations live limit 10 (AppViewModel.swift:296-304), openSearchResult/selectConversation (AppViewModel.swift:1508/340), SearchOverlayView prefix(12), SearchWorkspaceView Load More (only entry ContactsWorkspaceView:117), SearchIndex.sqlite metadata (exact.state/semantic.state, search_documents, indexed_conversations, semantic_embeddings).
- **Follow-ups:**
  - Fix search-result navigation into conversations not in the loaded sidebar list [out of lane] — openSearchResult sets selectedConversationID but selectedConversation resolves only against the 10 loaded live conversations, so clicking any years-old search hit dead-ends on the Select-a-conversation empty state; fetch/insert the conversation by ID when navigating from search (repository already has the record).
  - Make the paginated SearchWorkspaceView reachable (or add pagination to the overlay) [out of lane] — The only entry point to SearchWorkspaceView (Load More Results) is a button inside ContactsWorkspaceView which itself is unreachable without contact selection; the Cmd+Shift+P overlay caps at 12 results, so full-history result sets cannot be paged in the UI.
  - Contacts permission remains denied in System Settings [out of lane] — Sidebar shows raw phone numbers; the app Settings has a Request button but the denial is recorded in System Settings so it needs a user toggle (possibly same stale-TCC cdhash issue as FDA had).
- **By:** n1 · 2026-07-13T06:39:34.558Z

## n1: Resolved the DECISIONS.md jj merge conflict for the QA task
- **Did:** Resolved the 2-sided DECISIONS.md conflict from integrating the n1 real-app QA report. Both sides were duplicate write-ups of the same QA session; kept the fuller trunk-side entry (Interfaces + Follow-ups) and folded in the unique diff-side details: the `PageRequest(limit: isLiveData ? 10 : 200)` root-cause snippet, the broken "older threads stay reachable through search" intent note, and the TCC remove/re-add-keeps-stale-requirement detail. No code changed.
- **By:** n1 · 2026-07-13
## n1: Resolved the 2-sided DECISIONS.md jj merge conflict from integrating
- **Did:** Resolved the 2-sided DECISIONS.md jj merge conflict from integrating the n1 real-app QA report. Both sides were duplicate write-ups of the same QA session; kept the fuller trunk-side entry (with Interfaces and Follow-ups) and folded in the unique diff-side details: the PageRequest(limit: isLiveData ? 10 : 200) root-cause snippet, the broken older-threads-reachable-through-search intent note, and the TCC remove/re-add-keeps-stale-requirement detail. Appended a resolution note. jj resolve --list reports no conflicts; ./scripts/test.sh TEST SUCCEEDED.
- **Interfaces:** DECISIONS.md only - no code interfaces changed by this conflict resolution. Preserved n1 QA findings: navigation bug for conversations outside the 10 loaded sidebar rows, unreachable SearchWorkspaceView pagination, Contacts permission denial, FDA tccutil-reset fix confirming n2 stable signing.
- **Follow-ups:**
  - Fix search-result navigation into conversations not in the loaded sidebar list [out of lane] — openSearchResult sets selectedConversationID but selectedConversation resolves only against the 10 loaded live conversations, so old search hits dead-end on the Select-a-conversation empty state
  - Make paginated SearchWorkspaceView reachable or add pagination to the overlay [out of lane] — Load More Results is only reachable via ContactsWorkspaceView; the Cmd+Shift+P overlay caps at 12 results
- **By:** n1 · 2026-07-13T06:43:07.919Z
