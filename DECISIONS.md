...
## n1: Replaced the message-bubble context menu with a floating tapback pill
- **Did:** Removed the gray `.contextMenu` on `MessageBubble` and present an iMessage-style floating `TapbackPill` popover on right-click: the six standard tapbacks plus a trailing "+" that opens the existing `EmojiPickerPopover`. Right-click is captured by a new `RightClickCatcher` (`NSViewRepresentable`) whose `hitTest` claims the point only while a right-mouse event is routing, so primary-click text selection and clickable links pass through untouched. Taps call the existing `model.toggleReaction(_:on:)` / `model.toggleCustomReaction(_:on:)`; the active tapback reads as selected via a new `AppViewModel.currentUserReaction(on:)`. Per product decision, Copy Text / Open Thread / Add to Calendar were dropped from right-click (still reachable elsewhere). Confined edits to MessageBubble + new pill/overlay views in Components.swift; ComposerView untouched. build.sh + test.sh pass.
- **Interfaces:** `TapbackPill` and `RightClickCatcher`/`RightClickCatchingView` (Sources/i2MessageApp/Views/Components.swift); `TapbackPill.tapbackKinds` replaces the removed `MessageBubble.tapbackKinds`. `AppViewModel.currentUserReaction(on: Message) -> MessageReaction?` (Sources/i2MessageApp/Features/App/AppViewModel.swift). Test `testCurrentUserReactionTracksTapbackPillSelection` in Tests/i2MessageAppTests/UIModel/AppViewModelTests.swift.
- **Follow-ups:**
  - Manual real-app QA of the pill [out of lane] — Automated tests cover the model helper; a human should confirm in the running app that right-click shows the pill, tapping a tapback updates the ReactionCluster, "+" opens the emoji picker, and drag-selecting bubble text / clicking a link still work.
- **By:** n1 · 2026-07-08T23:35:00Z
