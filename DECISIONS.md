...
## n1: QA verified full-history search in the real app; found a navigation bug for old conversations
- **Did:** Real-app QA of full-history search passed for indexing and retrieval: exact index covers 671,249 documents / 2,439 conversations from the real 688k-message chat.db (metadata exact.state=complete), rebuild progress UI advances 0% → 100% → "Indexes are current", exact search returns messages from 2017/2018/2019/2020/2021 across multiple conversations, and semantic search returns relevant recent-topic snippets with similarity scores. Search-result navigation works and highlights the target message — but ONLY for conversations present in the loaded sidebar list.
- **BUG (open):** Clicking/entering a search result whose conversation is NOT among the loaded conversations (live mode loads only 10 recent — AppViewModel.loadConversations, `PageRequest(limit: dependencies.isLiveData ? 10 : 200)`) dead-ends on the "Select a conversation" empty state. Root cause: `AppViewModel.selectedConversation` (AppViewState resolution, AppViewModel.swift:131-139) resolves `selectedConversationID` against the loaded `conversations` array only; `openSearchResult` → `selectConversation` sets the ID and loads the transcript, but the detail router can't materialize a Conversation for it. The loadConversations comment says "older threads stay reachable through search" — that intent is currently broken. Fix idea: fetch the conversation record by ID (repository already has it) and insert it into `conversations` (or resolve `selectedConversation` via a by-ID lookup) when navigating from search.
- **Also found:** SearchWorkspaceView (the paginated search UI with "Load More Results") is unreachable — only entry point is ContactsWorkspaceView:117, and the contacts workspace itself is only reachable via contact selection. The Cmd+Shift-P overlay caps at 12 results with no pagination. "Results paginate" therefore can't be exercised in the running app.
- **FDA/TCC note:** a TCC FDA entry created for an old ad-hoc build pins a cdhash; toggling or even remove/re-add in System Settings kept the stale requirement. `tccutil reset SystemPolicyAllFiles dev.viraat.i2message` + fresh "+"-add stored the stable designated requirement (identifier + Apple Development cert), so FDA now survives rebuilds — n2's signing work is confirmed effective end-to-end.
- **By:** n1 · 2026-07-13
