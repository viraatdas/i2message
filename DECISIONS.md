...
- **Did:** Real-app QA of full-history search: PASSED for indexing, exact old-message retrieval, semantic search, and rebuild progress; FAILED (bug filed) for result-click navigation into conversations outside the 10 loaded sidebar rows. Evidence screenshots in session scratchpad (qa-*.png). Exact index: 671,249 docs / 2,439 conversations, exact.state=complete against the real 688,520-message chat.db. Rebuild progress UI advances 0% (Preparing local index / Indexing) to 100% (Indexes are current); a second rebuild short-circuits incrementally in <10s as designed. Exact search found terms occurring only in old messages: one term with 7 hits across 4 conversations dated Mar 2017-Feb 2019, another with 7 hits across 3 conversations dated Jul 2018-Jul 2020, a third from Dec 2021 - all matched sqlite ground truth counts. Semantic mode returned relevant recent-topic snippets with similarity scores (83%/76%/75%). Navigation control test: result click into a RECENT conversation navigates and highlights the exact message. Bug: result click into an OLD conversation lands on the Select-a-conversation empty state because AppViewModel.selectedConversation resolves only against the 10 loaded conversations. Also: SearchWorkspaceView (Load More pagination) is unreachable from any normal UI path, so pagination could not be exercised. FDA required a full tccutil reset + fresh add because the old grant pinned a stale cdhash; the new grant stores the stable designated requirement, confirming n2 signing works. No product code changed (jj working copy empty).
