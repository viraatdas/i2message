...
- Transcript detail with lazy `ScrollView`/`LazyVStack` rows, older-page loading that preserves the previous top visible message, explicit highlighted search result scrolling, local-send bottom scrolling, emoji tapback badges on bubble corners, and a per-bubble action pill (standard tapbacks, fixture-only custom emoji reactions, open thread, edit outgoing text, copy). The edit sheet compares the original and replacement, validates empty/unchanged/expired states, shows the live edit countdown and remaining edits, supports multiline text with Command-Return to continue, and explains the native Messages handoff as a three-step sequence. Fixture edits preserve their history locally; live edits stay read-only because macOS exposes no supported sent-message edit command. Quoted reply context, edited/failed states, and attachment transfer states are also rendered.
...
- The composer text field is backed by an `NSTextView` (`ComposerTextEditor`/`PasteInterceptingTextView`) so that ⌘V can be intercepted at the `paste(_:)` action. When the pasteboard holds image content (PNG/JPEG/TIFF/`NSImage`, or a file URL pointing at an image), the paste is diverted into a `DraftAttachment` chip via `AppViewModel.pasteImageAttachments(from:)` — each image's bytes are written to a real temp file — and no text is inserted; text-only pasteboards fall through to a normal paste.
- Docked thread panel with root-plus-replies transcript, its own `Reply in thread` composer with the same explicit emoji insertion control, Return-to-send, unread-thread activity state, focused close behavior, and a fixed-width opacity dock that avoids a competing slide transition. Reply chips and panel rows resolve outgoing messages to the current user's macOS Me-card photo; unresolved incoming senders use a neutral person icon instead of a question mark.
...
- Sidebar and transcript reads stay isolated from full-history search indexing, and avatar photos are held as decoded bounded-cache images so scrolling does not repeatedly decode the same bytes.
