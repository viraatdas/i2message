...
- Exact and semantic indexing starts at background priority after the first load path; the app does not wait for full indexing before showing conversations.
- Full-history indexing owns a separate read-only Messages database connection and thumbnail-free Contacts resolver. Conversation selection and transcript loads therefore never queue behind a corpus scan.
- Index progress is published in five-percent steps instead of once per indexed conversation, avoiding whole-window SwiftUI invalidations during long rebuilds.
...
- Attachment descriptions, decoded contact thumbnail results/misses, and date mention detection are bounded transient caches. Contact photos are decoded once rather than rebuilt from bytes during every SwiftUI refresh.
...
5. Rebuild local indexes from Diagnostics or Settings. While indexing is active, scroll well down the sidebar and switch between several conversations; selection and transcript loading must remain responsive. Then run exact search for a known phrase and open a result. Confirm the transcript routes to the highlighted message.
