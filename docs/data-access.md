...
`MessagesDataAccessPermissionManager` reports Full Disk Access by attempting a safe read-only open of the configured Messages database. macOS does not provide a programmatic Full Disk Access prompt, so `request(.fullDiskAccess)` opens System Settings and returns the current status.
