...
| Tapbacks, edit, undo send, delete, pin, mute | Unsupported for now | None | These are Apple-private Messages mutations or UI-only affordances without stable public automation contracts. | Open Messages.app for the native action. | Matrix documentation; add tests when a supported API appears |
